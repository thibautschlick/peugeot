<?php
	 ...
	 function affiche_texte_dyn($code_texte,$bouton = true) // $bouton =  false or true, pour afficher btn  de edit (false pour les liens menus par ex., car eux ont leur bouton pour editer le menu entier )
    {
        ...
        $texte_contenu_select = str_replace('1er','1<sup>er</sup>',$texte_contenu_select);
	...
	function membre_editeur_countries($email)
	{
        ...
        $req = $GLOBALS['bdd']->prepare("SELECT country FROM membres_countries WHERE email=:email"); // attention => donc en BDD = 1 ligne par email, avec 1 pays seulement, donc plusieurs lignes si plusieurs pays
